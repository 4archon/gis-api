...
func (s Server) postGSheetDoneVisits(response http.ResponseWriter, req *http.Request) {
	defer req.Body.Close()
	body, err := io.ReadAll(req.Body)
	if err != nil {
		log.Println(err)
	...
	data, err := s.DB.GetGSheetDoneVisits(options.Start, options.End)
	if err != nil {
		return
	}
...
func (s Server) postGSheetMedias(response http.ResponseWriter, req *http.Request) {
	defer req.Body.Close()
	body, err := io.ReadAll(req.Body)
	if err != nil {
		log.Println(err)
	...
	data, err := s.DB.GetGSheetMedias(options.Start, options.End)
	if err != nil {
		return
	}
